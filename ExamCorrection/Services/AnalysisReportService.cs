    ...
    public async Task<Result<(byte[] FileContent, string FileName)>> ExportStudentProgressToPdfAsync(DetailedStudentProgressPdfRequestDto request, string userId)
    {
        ...
        using (var pdf = new iText.Kernel.Pdf.PdfDocument(writer))
        {
            ...
            foreach (var report in reportsToGenerate)
            {
                ...
                // --- Student Info Banner ---
                var bannerTable = new iText.Layout.Element.Table(3).UseAllAvailableWidth().SetMarginBottom(25);
                Action<string, string, iText.Kernel.Colors.Color> addBannerInfo = (lbl, val, clr) => {
                    var cell = new iText.Layout.Element.Cell().SetBorder(new iText.Layout.Borders.SolidBorder(borderColor, 1f)).SetPadding(12).SetBackgroundColor(lightGrayBg).SetTextAlignment(iText.Layout.Properties.TextAlignment.CENTER);
                    cell.Add(new iText.Layout.Element.Paragraph(ArabicTextShaper.Shape(lbl)).SetFont(font).SetFontSize(8).SetFontColor(textSlate).SetBold().SetMarginBottom(4));
