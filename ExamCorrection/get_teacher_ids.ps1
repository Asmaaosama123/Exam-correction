...
try {
    ...
    $cmd.CommandText = "SELECT Id, Email FROM AspNetUsers WHERE Email IN ('menasalehalex@gmail.com', 'rogenayousryadel12345@gmail.com')"
    $reader = $cmd.ExecuteReader()
    while ($reader.Read()) {
        Write-Host "$($reader['Email']) -> $($reader['Id'])"
    }
} catch {
